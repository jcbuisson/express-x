...
describe('Full offline-first client ↔ server protocol', () => {
   ...
   test('server tombstone deletes stale server-only row without adding it to client', async () => {
      const modelName = `model${++dbCounter}`
      const { pglite, db, metaTable, modelTable } = await createTestDb(modelName)
      ...
      try {
         ...
         const rows = await db.select().from(modelTable).where(eq(modelTable.uid, 'r1'))
         assert.equal(rows.length, 0, 'stale server-only row must be deleted when metadata is tombstoned')
         assert.ok(!await model.db.values.get('r1'), 'tombstoned server-only row must not be added to client')
         assert.ok(!await model.db.metadata.get('r1'), 'tombstoned server-only metadata must not be added to client')
      } finally {
         await cleanup()
         pglite.close()
      }
   ...
   test('newer local tombstone tombstones orphaned live server metadata', async () => {
      const modelName = `model${++dbCounter}`
      const { pglite, db, metaTable, modelTable } = await createTestDb(modelName)
      ...
      try {
         const model = clientApp.createOfflineModel(modelName, ['label'])
         await model.db.metadata.add({ uid: 'r1', created_at: T0, deleted_at: T2, __dirty__: true })
         await model.addSynchroWhere({})

         await model.synchronizeAll()

         const rows = await db.select().from(modelTable).where(eq(modelTable.uid, 'r1'))
         assert.equal(rows.length, 0, 'server row must remain absent')
         const serverMeta = (await db.select().from(metaTable).where(eq(metaTable.uid, 'r1')))[0]
         assert.equal(new Date(serverMeta.deleted_at).getTime(), T2.getTime(), 'newer local tombstone should tombstone orphaned server metadata')
         assert.ok(!await model.db.values.get('r1'), 'local value should remain absent')
         assert.ok(!await model.db.metadata.get('r1'), 'acknowledged local tombstone should be cleared')
      } finally {
         await cleanup()
         pglite.close()
      }
   ...
   test('direct update older than server tombstone deletes stale server row', async () => {
      ...
      try {
         ...
         assert.equal(returnedValue == null, true, 'stale direct update should not return a live row')
         assert.equal(new Date(returnedMeta.deleted_at).getTime(), T2.getTime())

         const rows = await db.select().from(modelTable).where(eq(modelTable.uid, 'r1'))
         assert.equal(rows.length, 0, 'stale server row must be deleted when tombstone wins')
         const serverMeta = (await db.select().from(metaTable).where(eq(metaTable.uid, 'r1')))[0]
         assert.equal(new Date(serverMeta.deleted_at).getTime(), T2.getTime(), 'server tombstone must not be cleared')
      } finally {
         await cleanup()
         pglite.close()
      }
   ...
   test('direct update older than server update does not overwrite server row', async () => {
      const modelName = `model${++dbCounter}`
      const { pglite, db, metaTable, modelTable } = await createTestDb(modelName)
      ...
      try {
         ...
         assert.equal(returnedValue.label, 'server-new', 'stale direct update should return the newer server row')
         assert.equal(new Date(returnedMeta.updated_at).getTime(), T2.getTime())

         const rows = await db.select().from(modelTable).where(eq(modelTable.uid, 'r1'))
         assert.equal(rows[0]?.label, 'server-new', 'server row must survive stale direct update')
         const serverMeta = (await db.select().from(metaTable).where(eq(metaTable.uid, 'r1')))[0]
         assert.equal(new Date(serverMeta.updated_at).getTime(), T2.getTime(), 'server metadata must not be downgraded')
      } finally {
         await cleanup()
         pglite.close()
      }
   ...
   test('direct delete older than server tombstone does not downgrade tombstone', async () => {
      const modelName = `model${++dbCounter}`
      const { pglite, db, metaTable, modelTable } = await createTestDb(modelName)
      ...
      try {
         ...
         assert.equal(returnedValue == null, true, 'stale direct delete should not return a deleted server row')
   ...
   test('direct delete older than orphaned server metadata tombstones and removes local row', async () => {
      ...
      try {
         ...
         assert.equal(returnedValue == null, true, 'stale direct delete should not return a row when server metadata is orphaned')
         assert.equal(new Date(returnedMeta.deleted_at).getTime(), futureUpdate.getTime())

         const rows = await db.select().from(modelTable).where(eq(modelTable.uid, 'r1'))
         assert.equal(rows.length, 0, 'orphaned server row must remain absent')
         const serverMeta = (await db.select().from(metaTable).where(eq(metaTable.uid, 'r1')))[0]
         assert.equal(new Date(serverMeta.deleted_at).getTime(), futureUpdate.getTime(), 'orphaned live metadata should become a tombstone')
         ...
         for (let i = 0; i < 50; i++) {
            if (!await model.db.values.get('r1')) break
            await new Promise(resolve => setTimeout(resolve, 10))
         }

         assert.ok(!await model.db.values.get('r1'), 'delete acknowledgement tombstone should remove hidden local row')
         assert.ok(!await model.db.metadata.get('r1'), 'delete acknowledgement tombstone should remove local metadata')
      } finally {
         await cleanup()
         pglite.close()
      }
   ...
   test('direct create older than server update does not overwrite server row', async () => {
      const modelName = `model${++dbCounter}`
      const { pglite, db, metaTable, modelTable } = await createTestDb(modelName)
      ...
      try {
         ...
         assert.equal(returnedValue.label, 'server-new', 'stale direct create should return the newer server row')
         assert.equal(new Date(returnedMeta.updated_at).getTime(), T2.getTime())

         const rows = await db.select().from(modelTable).where(eq(modelTable.uid, 'r1'))
         assert.equal(rows[0]?.label, 'server-new', 'server row must survive stale direct create')
         const serverMeta = (await db.select().from(metaTable).where(eq(metaTable.uid, 'r1')))[0]
         assert.equal(new Date(serverMeta.updated_at).getTime(), T2.getTime())
      } finally {
         await cleanup()
   ...
   test('direct create older than orphaned server metadata tombstones and removes optimistic row', async () => {
      const modelName = `model${++dbCounter}`
      const { pglite, db, metaTable, modelTable } = await createTestDb(modelName)
      ...
      try {
         ...
         assert.equal(returnedValue == null, true, 'stale direct create should not return a row when server metadata is orphaned')
         assert.equal(new Date(returnedMeta.deleted_at).getTime(), T2.getTime())
         ...
         assert.ok(!await model.db.values.get('r1'), 'sync should remove optimistic row after orphaned metadata tombstone')
         assert.ok(!await model.db.metadata.get('r1'), 'sync should remove optimistic metadata after orphaned metadata tombstone')
      } finally {
         await cleanup()
         pglite.close()
      }
   ...
   test('direct create older than server tombstone does not recreate server row', async () => {
      const modelName = `model${++dbCounter}`
      const { pglite, db, metaTable, modelTable } = await createTestDb(modelName)
      ...
      try {
         ...
         assert.equal(returnedValue == null, true, 'stale direct create should not return a recreated row')
         assert.equal(new Date(returnedMeta.deleted_at).getTime(), T2.getTime())

         const rows = await db.select().from(modelTable).where(eq(modelTable.uid, 'r1'))
         assert.equal(rows.length, 0, 'server tombstone must survive stale direct create')
         const serverMeta = (await db.select().from(metaTable).where(eq(metaTable.uid, 'r1')))[0]
         assert.equal(new Date(serverMeta.deleted_at).getTime(), T2.getTime())
      } finally {
         await cleanup()
         pglite.close()
      }
   ...
   test('sync addDatabase tombstone response ignores returned value', async () => {
      const modelName = `model${++dbCounter}`
      const futureDelete = new Date(Date.now() + 60_000)

      const { clientApp, cleanup } = await createTestContext(serverApp => {
         serverApp.createService(modelName, {
            createWithMeta: async (uid) => [{ uid, label: 'stale-server-row' }, { uid, created_at: T0, updated_at: null, deleted_at: futureDelete }],
            updateWithMeta: async () => {},
            deleteWithMeta: async () => {},
            findMany: async () => [],
         })
      ...
      try {
         ...
         assert.ok(!await model.db.values.get('r1'), 'sync addDatabase tombstone response should remove local value even if a value is returned')
         assert.ok(!await model.db.metadata.get('r1'), 'sync addDatabase tombstone response should remove local metadata even if a value is returned')
      } finally {
         await cleanup()
      }
   })
   ...
   test('direct update acknowledgement removes local row when server returns tombstone', async () => {
      ...
      const updateCanReturn = new Promise(resolve => { updateRelease = resolve })
      ...
      try {
         ...
         assert.ok(!await model.db.values.get('r1'), 'stale update tombstone response should remove local value')
         assert.ok(!await model.db.metadata.get('r1'), 'stale update tombstone response should remove local metadata')
      } finally {
         await cleanup()
      }
   })

   test('direct update acknowledgement ignores value when server returns tombstone', async () => {
      const modelName = `model${++dbCounter}`
      const futureDelete = new Date(Date.now() + 60_000)
      let resolveUpdateStarted
      let updateRelease
      ...
      try {
         ...
         assert.ok(!await model.db.values.get('r1'), 'tombstone update acknowledgement should remove value even if a value is returned')
         assert.ok(!await model.db.metadata.get('r1'), 'tombstone update acknowledgement should remove metadata even if a value is returned')
      } finally {
         await cleanup()
      }
   })

   test('sync updateDatabase tombstone response removes local row', async () => {
      const modelName = `model${++dbCounter}`
      const futureDelete = new Date(Date.now() + 60_000)
      ...
      const { clientApp, cleanup } = await createTestContext(serverApp => {
         serverApp.createService(modelName, {
            createWithMeta: async () => {},
            updateWithMeta: async (uid) => [null, { uid, created_at: T0, updated_at: null, deleted_at: futureDelete }],
            deleteWithMeta: async () => {},
            findMany: async () => [],
         })
         serverApp.createService('sync', {
   ...
   test('sync updateDatabase tombstone response ignores returned value', async () => {
      const modelName = `model${++dbCounter}`
      const futureDelete = new Date(Date.now() + 60_000)
      ...
      const { clientApp, cleanup } = await createTestContext(serverApp => {
         serverApp.createService(modelName, {
            createWithMeta: async () => {},
            updateWithMeta: async (uid) => [{ uid, label: 'stale-server-row' }, { uid, created_at: T0, updated_at: null, deleted_at: futureDelete }],
            deleteWithMeta: async () => {},
            findMany: async () => [],
         })
         serverApp.createService('sync', {
      ...
      try {
         ...
         assert.ok(!await model.db.values.get('r1'), 'sync updateDatabase tombstone response should remove local value even if a value is returned')
         assert.ok(!await model.db.metadata.get('r1'), 'sync updateDatabase tombstone response should remove local metadata even if a value is returned')
      } finally {
         await cleanup()
      }
   })
   ...
   test('stale direct update rejection does not roll back newer local update', async () => {
      ...
      const { clientApp, cleanup } = await createTestContext(serverApp => {
         serverApp.createService(modelName, {
            updateWithMeta: async (uid, data, updated_at) => {
               ...
               if (updateCount === 1) {
                  await new Promise(resolve => { firstUpdateRelease = resolve })
                  throw new Error('first update rejected late')
               }
               return [{ uid, ...data }, { uid, created_at: T0, updated_at, deleted_at: null }]
            },
            createWithMeta: async () => {},
      ...
      try {
         ...
         assert.equal(value.label, 'second', 'older rejection must not roll back the newer update')
         assert.equal(meta.__dirty__, false, 'newer acknowledged update should remain clean')
         assert.equal(new Date(meta.updated_at).getTime() > T0.getTime(), true)
      } finally {
         await cleanup()
   ...
   test('stale sync updateClient does not overwrite newer local update', async () => {
      ...
      const { clientApp, cleanup } = await createTestContext(serverApp => {
         ...
         serverApp.createService('sync', {
            go: async () => {
               ...
               return {
                  addClient: [],
                  updateClient: [[{ uid: 'r1', label: 'server-old' }, { uid: 'r1', created_at: T0, updated_at: T1, deleted_at: null }]],
                  deleteClient: [],
                  addDatabase: [],
                  updateDatabase: [],
               }
      ...
      try {
         ...
         assert.equal(value.label, 'local-new', 'stale sync updateClient must not overwrite newer local update')
         assert.equal(meta.__dirty__, false, 'newer acknowledged update should remain clean')
         assert.equal(new Date(meta.updated_at).getTime() > T1.getTime(), true)
      } finally {
         await cleanup()
      }
   })
   ...
   test('stale sync deleteClient does not delete newer local update', async () => {
      ...
      try {
         ...
         assert.equal(value.label, 'local-new', 'stale sync deleteClient must not delete newer local update')
         assert.ok(meta, 'metadata must remain after stale sync deleteClient')
         assert.equal(meta.__dirty__, false, 'newer acknowledged update should remain clean')
      } finally {
         await cleanup()
      }
   ...
   test('wherePredicate handles persisted Date range bounds consistently with server sync scope', async () => {
      const modelName = `model${++dbCounter}`
      const pglite = new PGlite()
      await pglite.exec(`
         CREATE TABLE metadata (uid TEXT PRIMARY KEY, created_at TIMESTAMP, updated_at TIMESTAMP, deleted_at TIMESTAMP);
      ...
      try {
         const model = clientApp.createOfflineModel(modelName, ['label', 'start'])
         await model.db.values.add({ uid: 'before', label: 'before-range', start: T0 })
         await model.db.metadata.add({ uid: 'before', created_at: T0, __dirty__: true })

         const persistedWhere = { start: { gte: T1.toISOString() } }
         assert.deepEqual(await model.findWhere(persistedWhere), [], 'persisted Date range should not match a Date before the lower bound')

         await model.addSynchroWhere(persistedWhere)
         await model.synchronizeAll()

         const serverRows = await db.select().from(modelTable)
         assert.equal(serverRows.length, 0, 'Date-range sync must not push local rows outside a serialized Date lower bound')
      } finally {
         await cleanup()
         pglite.close()
      }
